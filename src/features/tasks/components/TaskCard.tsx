...
export function TaskCard({ task, labels = [], onClick, onAgentClick, onCancel, isNew }: TaskCardProps) {
  ...
  return (
    <div
      ...
      `}
      style={
        isWorking && task.assigned_to
          ? {
              borderColor: `color-mix(in srgb, ${agentColor(task.assigned_to)} 30%, transparent)`,
              boxShadow: `0 0 20px color-mix(in srgb, ${agentColor(task.assigned_to)} 12%, transparent)`,
            }
          : undefined
      }
    >
